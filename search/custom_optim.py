...
class Custom_SGD(Optimizer):
    ...
    def __init__(self, params, lr=required, eigen_powered=required, momentum=0, dampening=0,
        ...
        self.eigen_powered = eigen_powered

    def compute_penalty(self, t):
        "Check if constraints satisfied, return 1 if not satisfied and 0 otherwise"
        hypothetical_params = self.params - t*self.d
        constraint = [torch.sum(self.params * self.eigen_powered[:, j]).reshape(-1) for j in range(self.eigen_powered.shape[1])]
        constraint = torch.cat(constraint)
        result = int((constraint < 0).any())
        return result
    
    def binary_search(self):
        result = self.compute_penalty(1.)
        if result:
            low = 0.
            high = 1.
            mid = 0
            while high - low >= 1e-4:
                mid = (high + low) / 2
                result = self.compute_penalty(mid)
